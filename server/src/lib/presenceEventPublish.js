 ...
 * - meaningful state change → analytics.presence_changed (immediate, always)
 * - ordinary same-state TTL refresh → coalesced analytics.session_heartbeat
 *   (and suppressed under pool pressure so critical paths keep capacity)
...
export function publishAfterLivePresenceUpsert(
  ...
  // Ordinary TTL refresh: never starve verify/payment under pressure.
